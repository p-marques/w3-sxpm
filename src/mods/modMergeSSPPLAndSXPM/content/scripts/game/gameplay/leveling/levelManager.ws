...
class W3LevelManager
{
	...
	private var mSetSkillPointsPerLevel : SetSkillPointsPerLevelManager; // SetSkillPointsPerLevel
    private var setXpMultiplierManager : SetXpMultiplierManager; // SetXpMultiplier
	...
	public function AddPoints(type : ESpendablePointType, amount : int, show : bool )
	{
        ...
        // Set XP Multiplier
		if (!setXpMultiplierManager)
			setXpMultiplierManager = new SetXpMultiplierManager in this;

		if (type == EExperiencePoint && setXpMultiplierManager.GetIsModOn())
		{
			if (setXpMultiplierManager.GetStopAllXp())
				return;
			else
				amount = setXpMultiplierManager.GetModifiedAmount(amount);
	...
	public function GainLevel( show : bool ) : bool
	{
		...
		if(mSetSkillPointsPerLevel.GetIsModOn())
			AddPoints(ESkillPoint, mSetSkillPointsPerLevel.GetSkillPointsPerLevelValue(), show);
